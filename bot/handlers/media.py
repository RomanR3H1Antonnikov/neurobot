...
def _audio_ref_hint(audio_names: list[str], max_refs: int) -> str:
    count = len(audio_names)
    if not count:
        return (
            f"🎵 Аудио\n\n"
            f"Здесь ты загружаешь свой звук, который хочешь использовать в видео "
            f"(музыка, голос, эффекты).\n\n"
            f"Это не то же самое, что «Звуковое сопровождение» — тот раздел управляет "
            f"звуком, который нейросеть сгенерирует сама.\n\n"
            f"Можно добавить до {max_refs} файлов. Когда закончишь — нажми «Назад»."
        )
...
@router.callback_query(MediaStates.enter_reference, F.data == "media:delete_audio_refs_all")
async def delete_audio_refs_all(callback: CallbackQuery, state: FSMContext) -> None:
    data = await state.get_data()
    max_refs = data.get("model_max_audio_refs", 0)
    await state.update_data(
        audio_reference_file_ids=None,
        audio_reference_file_names=None,
    )
    await _update_sref_status(
        callback.bot, callback.message.chat.id, state,
